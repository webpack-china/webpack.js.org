...
+ `comments`: 保留注释。 默认: `/^\**!|@preserve|@license|@cc_on/`, 不匹配为移除所有注释。可以接受函数，带有属性测试（正则）和值的对象。
...
+ `babel`: 通过一个自定义的babel核心代替。 `require("babel-core")`
+ `minifyPreset`: 通过自定义的minify preset代替 - `require("babel-preset-minify")`.
...
+ webpack loader对单个文件进行操作，并且minify preset作为一个webpack loader将会把每个文件视为在浏览器全局范围内直接执行（默认情况下），并且不会优化顶层范围内的某些内容。要在文件的顶层范围内进行优化，请在minifyOptions中设置 `topLevel：true`。
